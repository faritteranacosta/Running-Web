...
function updatePagination() {
  ...
  if (startPage > 1) {
    ...
    firstPageBtn.className = `px-3 py-1 border border-gray-300 rounded-md ${
      1 === currentPage
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    pageBtn.className = `px-3 py-1 border rounded-md ${
      i === currentPage
        ? "bg-primary-600 text-white"
  ...
  if (endPage < totalPages) {
    ...
    lastPageBtn.className = `px-3 py-1 border border-gray-300 rounded-md ${
  ...
  paginationControls.insertBefore(
    pageNumberContainer,
  ...
  productsToShow.forEach((product, index) => {
    ...
    <div class="flex justify-center space-x-2">
        <button aria-label="Editar producto ${
          product.name
        }" class="text-yellow-500 hover:text-yellow-700" onclick="editProduct(${index})">
