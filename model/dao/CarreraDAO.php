...
class CarreraDAO {
 ...
 public function obtenerCarreraPorId($id_carrera) {
    $sql = "SELECT c.*, e.hora AS hora_evento, e.nombre AS evento_nombre, e.fecha AS evento_fecha, e.descripcion AS evento_descripcion, e.id_patrocinador
            FROM carrera c
    ...
    if (count($result) > 0) {
        ...
        // Obtener el nombre del patrocinador solo si hay id_patrocinador y la conexión está activa
        $patrocinador_nombre = null;
        if (!empty($row['id_patrocinador']) && $this->dataSource && $this->dataSource->getConnection()) {
            $sqlPat = "SELECT nombre FROM patrocinador WHERE id_patrocinador = ? LIMIT 1";
            $patResult = $this->dataSource->ejecutarConsulta($sqlPat, [$row['id_patrocinador']]);
            if ($patResult && isset($patResult[0]['nombre'])) {
                $patrocinador_nombre = $patResult[0]['nombre'];
            }
        }
        $carrera->patrocinador_nombre = $patrocinador_nombre;
        return $carrera;
    }
    return null;
}
